...
if ChannelID<>0 And ChannelID<>""  then
	...
	if rs.eof then Response.Write "<option value='0'> 你没设分类</option>"
	while not rs.eof
...
Else
			...
			RS.Open ("select Cate_ID,Cate_Name from Blog_Category Where Cate_outLink=0 AND Cate_Lock=0 Order BY Cate_Order ASC"),conn, 1, 1
			Do While Not RS.EOF
			 ID = Trim(RS(0))
...
'*************************************************************************************
	...
	Public Function GetInfoID_CMS(ChannelID)
	   ...
	   Select Case ChannelID
		 Case 1
		   TableNameStr = "Select Cate_ID From blog_Category Where Cate_ID='"
		 Case 2
...
Sub Admin_ShowClass_Name(ChannelID,ClassID)   
   ...
   IF CMSDataBase=1 Then
   		 Sqlc ="Select top 1 Cate_Name from Blog_Category Where Cate_ID="& ClassID &"" 
		 Set RsC=server.CreateObject("adodb.recordset") 
