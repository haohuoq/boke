...
Class FunCls
	...
	Sub Show_Top()
		...
		Response.Write "<title>SK采集系统For PJblog By http://www.eexe.net</title>" & vbcrlf
		...
		Response.Write "    <td height=""30"" colspan=""3"" bgcolor=""f3f3f3"">"
		%>
		 <!--#include file="../CJ_Pjblog.asp"-->
         
		 <%
		
		Response.Write "<b>说明：</b><br>&nbsp;&nbsp;①、第一次使用本功能，请修改<a href='"& CJFileName &"?action=config'><font color=blue>采集基本设置</font></a>；<br>" & vbcrlf
		...
		Response.Write "    <td width=""600"" bgcolor=""f3f3f3""><a href="& CJFileName &">管理首页</a> | <a href="""& CJFileName &"?action=add&Colleclx="& Colleclx &""">添加新项目</a> | <a href='"& CJFileName &"?action=config&ChannelID=0'>采集基本设置</a> "
		IF CMSDataBase<>1 Then
			Response.Write "|<a href=""sk_class.asp?Colleclx="& Colleclx &""">分类设置 </a>"
		End if
		...
		IF CMSDataBase=1 Then
			'Call Showclass_Cms(ClassID,Colleclx) 
		Else
	...
	'作  用：显示频道栏目分类单机版
	'==================================================	
	Sub Showclass_Cms(FolderID,ChannelID)
		Dim CJFileName : CJFileName = GetItemConfig("FileName",Colleclx)
		if ChannelID=0 then
		Response.Write"<option selected value='0'>你没设分类</option>"
		Else
					...
					Do While Not RS.EOF
					 ...
					 FolderName = Trim(RS(1))
						 If FolderID = ID Then
						   TreeStr = TreeStr & "<option selected value='"& CJFileName &"?DclassID=" & ID & "'>" & FolderName & "</option>"
						 Else
						  TreeStr = TreeStr & "<option value='"& CJFileName &"?DclassID=" & ID & "'>" & FolderName & " </option>"
						 End If
