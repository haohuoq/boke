...
'==================================================================================================
'===================================
'文件最后改于2007-12-30 by http://eexe.net
'================================================
' 软件名称：SK信息采集管理系统
...
'===================================================================================================
Dim SKThumb
Set SKThumb=New Sk_Thumb
Dim sk
Set sk = New SK_cj
'-----------整合版调用----------------
'Private KSCMS : Set KSCMS=New publiccls
'--------------------------------------
...
End if 	  
      ...
      If FoundErr<>True Then
         ...
         ElseIf KeyType=2 Then
            ...
            End If
         End If
         If Key="" or Key="$False$" or key=Null Then
            Key=""
         End If
		 IF IsNull(Key)=True then Key=""
...
End Sub
'以下是blog文章导入数据
'by EexeNet http://eexe.net
Sub SaveDate_Passed'保存数据
	dim rslb,DownID,CMSRS,UploadFiles_1
	Select Case Colleclx
	Case 1'--文章数据库
		Set rslb=Conn.Execute("select top 1 * from blog_Category Where Cate_ID=" & ClassID &"") 
		if not rslb.eof then
		   Set CMSRS=Server.CreateObject("ADODB.RECORDSET")
		   CMSRS.Open "Select top 1 * From blog_Content Where log_Title='" & Title & "' And log_CateID=" & ClassID & "",Conn,1,3
		   If CMSRS.EOF Then
			   CMSRS.addnew
			  
			   CMSRS("log_CateID")=ClassID'目标ID	  
			   'CMSRS("Keywords")=Key
			   CMSRS("log_Title")=Title
			
			   CMSRS("log_Content")=Content		    
			   CMSRS("log_Intro")= left(Content,300)
			   CMSRS("log_Author")=Author
			   CMSRS("log_from")=CopyFrom
			   CMSRS("log_fromUrl")=CopyFrom
			  ' CMSRS("log_Level")=GetStars(Stars)
			   CMSRS("log_viewNums")=Hits '点击数
			   CMSRS("log_postTime")=UpDateTime
			   CMSRS("log_ubbFlags")="000111"
			   CMSRS("log_edittype")="0"
			   ' CMSRS("TemplateID")=rslb("TemplateID")
			   'CMSRS("ArticleFsoType")=rslb("ArticleFsoType")
			   'CMSRS("Fname")=CMSRS("ID") & rslb("FnameType")
			  ' CMSRS("Fname")=KSCMS.GetFileName(rslb("FsoType"),now,"") & rslb("FnameType")
			   'CMSRS("RefreshTF")=0
			 ...
			 '  CMSRS("Popular") = 0
			  ...
			  ' CMSRS("OrderID") = 1
			   CMSRS.update
			 '  If Left(Ucase(CMSRS("Fname")),2)="ID" Then
'					   CMSRS.MoveLast
'					   CMSRS("Fname") = CMSRS("ID") & rslb("FnameType")
'					   CMSRS.Update
'					  End If
			...
			CMSRS.Open "Select top 1 * From KS_Photo Where Title='" & Title & "' And Tid='" & ClassID & "'",Conn,1,3
	   		If CMSRS.EOF Then
				...
				if x_tp =1 then 
					...
					picpath=Replace(TypeUrlArray_2(0),"|","")
				...
				IF Isnull(Content)=False then Content=""
				CMSRS("PictureContent")=Content
				IF Isnull(key)=False then Author=""
				CMSRS("Author")=Author
	    ...
	    if not rslb.eof then  
			...
			CMSRS.Open "Select top 1 * From KS_download Where Title='" & Title & "' And Tid='" & ClassID & "'",Conn,1,3
	   	   If CMSRS.EOF Then
				...
				CMSRS("TemplateID")=rslb("TemplateID")           '模板ID
				CMSRS("Fname")=CMSRS("ID") & rslb("FnameType")
   ...
   if  Colleclx =2 then 
	   ...
	   rs("Title")=Title
	   	If PhotoPaingType=1 Then
			rs("FileUrls")=PicUrls
		Else
			rs("FileUrls")=Coll_Skcj.FormatRemoteUrl
		End if
		  rs("Content")=Content
	   rs("Keyword")=Key
	   rs("Hits")=Hits
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="tableBorder">
  <tr class="tdbg"> 
	...
	if Colleclx=1 then Response.Write "<td height=""30""><a href=""sk_GetArticle.asp"">管理首页</a> >> 新闻采集</td>"  
	if Colleclx=2 then Response.Write "<td height=""30""><a href=""SK_Getphoto.asp"">管理首页</a> >> 新闻采集</td>"  
	if Colleclx=3 then Response.Write "<td height=""30""><a href=""SK_Getphoto.asp"">管理首页</a> >> 新闻采集</td>"
	rs.close
	set rs=nothing
	 %>
  </tr>  
